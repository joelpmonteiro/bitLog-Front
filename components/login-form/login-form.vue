...
export default {
  ...
  methods: {
    /* Nomeando a mutations: nome da mutations real */
    async login() {
      // eslint-disable-next-line no-console
      //   console.log('clicou enter', this.$axios)
      try {
        ...
        if (response.status === 200) {
          ...
          // this.save({ name: response.data.name })

          alert(response.data.msg)
          setTimeout(() => this.$router.push('/dashboard/profile'), 1200)
        }
      } catch (e) {
        ...
        } else alert('Erro interno no sistema!')
      }
    },
  },
}
